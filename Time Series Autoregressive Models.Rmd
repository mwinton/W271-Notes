...
$$ x_t = [\delta +]\ \alpha_1 x_{t-1} + \alpha_2 x_{t-2} + ... + \alpha_p x_{t-p} + w_t$$
Note: $\delta$ is a "drift" parameter, which we haven't covered yet.  (Not totally sure it belongs in the AR model.)

With the backward shift operator, we can rewrite this as:
$$ x_t = [\delta +]\ \alpha_1 B + \alpha_2 B^2 + ... \alpha_p B^p + w_t$$
Rearrange to get $w_t$ by itself:
$$ (1 - \alpha_1 B - \alpha_2 B^2 - ... \alpha_p B^p) x_t = [\delta +]\  w_t$$

We define $\theta_p(B)$ as this coefficient of $x_t$:
$$ \theta_p(B) =  (1 - \alpha_1 B - \alpha_2 B^2 - ... \alpha_p B^p) $$
which gives:
$$ \theta_p(B) x_t = [\delta +]\ w_t$$
...
$$ E(x_t) = \frac{\delta}{1-\alpha_1 - \alpha_2 - ... - \alpha_p}$$
