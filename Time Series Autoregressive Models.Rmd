...
$$ x_t = \alpha_0 + \alpha_1 B + \alpha_2 B^2 + ... \alpha_p B^p + w_t$$
Rearrange to get $w_t$ by itself:
$$ (1 - \alpha_1 B - \alpha_2 B^2 - ... \alpha_p B^p) x_t = \alpha_0 + w_t$$

We define $\alpha(B)$ as this coefficient of $x_t$:
$$ \alpha(B) =  (1 - \alpha_1 B - \alpha_2 B^2 - ... \alpha_p B^p) $$
which gives:
$$ \alpha(B) x_t = \alpha_0 + w_t$$

The _characteristic equation_ is defined by $\alpha(B) = 0$:

$$ \alpha(B) =  (1 - \alpha_1 B - \alpha_2 B^2 - ... \alpha_p B^p) = 0 $$
...
The mean of this model is given by:
$$ E(x_t) = \frac{\alpha_0}{1-\alpha_1 - \alpha_2 - ... - \alpha_p}$$

### Alternate terminologies

In the lectures, we sometimes see these alternate terminologies:

- $\phi(B)$ or $\Phi(B)$ instead of $\alpha(B)$
...
```{r}
x2 <- w <- rnorm(1000)  # not clear we need this line
x2 <- arima.sim(n=1000,list(ar=c(1.5, -0.9), ma=0))
str(x2)
summary(x2)
...
```{r}
x1 <- w <- rnorm(1000)
x1 <- arima.sim(n=1000, list(ar=c(0.7), ma=0))
str(x1)
summary(x1)
