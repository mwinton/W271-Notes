...
We can also test the serial correlation between the residuals with the Portmanteau Test:

```{r}
serial.test(var2_fit, lags.pt=12, type = 'PT.adjusted')
```

The highly statistically significant p-value tells us that we strongly reject the null hypothesis. (TODO:figure out what the null hypothesis is.)

Now, do 4-week and 8-week forecasts with this model:
