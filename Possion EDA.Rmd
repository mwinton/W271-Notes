...
This plot command plots one plot per predictor.  Note that connecting lines for the categorical predictors don't have any real meaning.

*Assets Effect plot*: y-axis is on log scale (scale of linear predictor).

## Model Diagnostics

The diagnostic plots for a Poisson model are different than for linear regression.  These plots use Pearson residuals.  There is one plot per explanatory variable, as well as one for the linear predictor.  We want to see that there are no systematic patterns between residuals and the explanatory variables.

```{r}
residualPlots(poisson_fit, layout=c(2,2))
```

*Observations:* not much pattern observed here.  The *lack of fit test* results are also displayed (only meaninigful for continuous variables, otherwise `NA`).  Here, we reject the null hypothesis that there's a relationship between Pearson residuals and `log2(assets)`.
